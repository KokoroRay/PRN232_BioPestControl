...
import type { Product, CreateProductRequest } from '../../types/catalog';
...
const ProductsPage: React.FC = () => {
  ...
  const displayed = useMemo(() => {
    ...
    if (statusFilter === 'banned') list = list.filter((p) => !p.isActive);
    return list;
  ...
  return (
    <div className="admin-page">
                ...
                <tr key={p.id}>
                  ...
                  {canManageCatalog && (
                    <td className="actions-cell">
                      ...
                      <button type="button" className="btn-icon danger" onClick={() => setDeleteId(p.id)}>
                        <Trash2 size={18} />
                      </button>
                    </td>
                  )}
      ...
      <Drawer open={drawerOpen} title={editId ? 'Edit Product' : 'Add Product'} onClose={() => setDrawerOpen(false)} wide>
        <form onSubmit={handleSubmit} className="form-stack">
          <label>SKU *<input value={form.sku} onChange={(e) => setForm({ ...form, sku: e.target.value })} required /></label>
          <label>Name *<input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required /></label>
          <label>Unit Price *<input type="number" min={0} step={1000} value={form.unitPrice} onChange={(e) => setForm({ ...form, unitPrice: Number(e.target.value) })} required /></label>
          <label>Category *
          ...
          <label>Chemical Profile
            <select value={form.chemicalProfileId ?? ''} onChange={(e) => setForm({ ...form, chemicalProfileId: e.target.value ? Number(e.target.value) : undefined })}>
      ...
      <ConfirmModal open={deleteId != null} title="Delete Product" message="Delete this product?" danger confirmLabel="Delete" onConfirm={handleDelete} onCancel={() => setDeleteId(null)} />
    </div>
  );
};
